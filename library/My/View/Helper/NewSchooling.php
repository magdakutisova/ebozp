<?php
class My_View_Helper_NewSchooling extends Zend_View_Helper_FormElement{
	
	protected $html = '';
	
	public function newSchooling($name, $value = null, $attribs = null){
		$this->html = '';
		...
		$this->html .= '</tr><tr>';
		$this->html .= '<td colspan="3">' . $helperText->formText($name . '[schooling]', $schooling) . '</td>';
		...
		if($canViewPrivate){
			$this->html .= '<td colspan="2"><label for="' . $name . '[private]">Soukromá poznámka ke školení</label></br>' . $helperText->formText($name . '[private]', $private) . '</td><td></td>';
		}
		else{
